...
pub async fn add_hobby(State(pg_pool) : State<PgPool>,
Json(add_hobby_to_user_data) : Json<AddHobbyToUser>)
    -> Result<(StatusCode, String), (StatusCode, String)> {
match UserModel::add_hobby_to_user(&add_hobby_to_user_data.user_name, &add_hobby_to_user_data.hobby_name, &pg_pool).await {
    Ok(_) => Ok((StatusCode::CREATED, "Hobby added!".to_string())),
    Err(error) =>{
        error!("Error at adding hobby [{}] to user [{}] the error is [{}]",
            add_hobby_to_user_data.hobby_name,
            add_hobby_to_user_data.user_name,
            error.to_string());
        Err((StatusCode::INTERNAL_SERVER_ERROR, "[USER_003] Something went wrong!".to_string()))
    }
}
}